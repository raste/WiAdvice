...
namespace UserInterface
{
    public partial class Rules : BasePage
    {
        ...
        {
            CheckUser();                        // Checks user , if admin is logged shows rules ID`s
            ShowInfo();
        }
        ...
        private void ShowInfo()
        {
            ...
            SiteNews about = siteText.GetSiteText(objectContext, "aboutRules"); ;
            if (about == null)
            {
        ...
        private void FillData()
        {
            ...
            List<SiteNews> rules = siteTexts.GetRules(objectContext, false).ToList();

            FillTblRules(rules);
            FillTblRulesDescr(rules);
        }


        private void FillTblRules(List<SiteNews> rules)
        {
            tblRules.Rows.Clear();
            ...
            if (rules.Count<SiteNews>() > 0)
            {
                foreach (SiteNews rule in rules)
                {
                    ...
                    ruleLink.NavigateUrl = string.Format("{0}#{1}", GetUrlWithVariant("Rules.aspx"), rule.linkID);

                }
            }
        }

        private void FillTblRulesDescr(List<SiteNews> rules)
        {
            System.Web.UI.HtmlControls.HtmlGenericControl mainDiv = new System.Web.UI.HtmlControls.HtmlGenericControl("div");
            phRules.Controls.Add(mainDiv);
            ...
            if (count > 0)
            {
                ...
                foreach (SiteNews rule in rules)
                {
                    ...
                    goToTop.NavigateUrl = GetUrlWithVariant("Rules.aspx#");
            ...
            else
            {
                ...
                Label descrLbl = new Label();
                descrLbl.Text = GetLocalResourceObject("noEnteredRules").ToString();
